...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class TilesetEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            else
            {
                ...
                if (s_atlasSettingsFoldout)
                {
					...
					tileset.TilePxSize = EditorGUILayout.Vector2Field ("Tile Size (pixels)", tileset.TilePxSize);
					tileset.SliceOffset = EditorGUILayout.Vector2Field(new GUIContent("Offset", "The offset position in pixels to start slicing tiles from the atlas texture"), tileset.SliceOffset);
					tileset.SlicePadding = EditorGUILayout.Vector2Field (new GUIContent ("Padding", "The separation in pixels between tiles"), tileset.SlicePadding);

					tileset.TilePxSize = _GetPositiveIntVect2(tileset.TilePxSize, 1, 1);
					tileset.SliceOffset = _GetPositiveIntVect2(tileset.SliceOffset, 0, 0);

					// CUSTOM
					string atlasPath = AssetDatabase.GetAssetPath (tileset.AtlasTexture);
					string atlasName = tileset.AtlasTexture.name;
					string saveFolder = TextureToolWindow.GetFolderPath (tileset.AtlasTexture);
					string suffix = atlasPath.Substring (atlasPath.LastIndexOf ("."));
					bool isSplitted = atlasPath.Contains ("_splitted.");
					bool split = EditorGUILayout.Toggle("Split", isSplitted);
					...
					if (split && !isSplitted) {
						tileset.SlicePadding = _GetPositiveIntVect2(tileset.SlicePadding, split ? 8:0, split ? 8:0);

						string savePath = saveFolder + "/" + atlasName + "_splitted" + suffix;
						Texture2D newTexture2D = TextureToolWindow.SliceTexture (tileset.AtlasTexture, tileset.SliceOffset, tileset.TilePxSize, tileset.SlicePadding, true, savePath);
						tileset.AtlasTexture = newTexture2D;
					} else if (!split && isSplitted){
						string srcPath = saveFolder + "/" + atlasName.Replace ("_splitted", "") + suffix;
						Texture2D loadTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(RailGunEditorUtility.GetAssetPathFromDiskPath(srcPath));
		...
		private Vector2 _GetPositiveIntVect2(Vector2 v,int minX,int minY)
        {
			return new Vector2(Mathf.Max(minX, (int)v.x), Mathf.Max(minY, (int)v.y));
        }
