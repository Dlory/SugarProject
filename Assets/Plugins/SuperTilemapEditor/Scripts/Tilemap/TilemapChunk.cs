...
//		}
        ...
        void OnEnable()
        {
            ...
            if (Application.isPlaying)
            {
                // Refresh only if Mesh is null (this happens if hideFlags == DontSave)
                m_needsRebuildMesh = m_meshFilter.sharedMesh == null;
				//总有parentTilemap为空的情况。
				eColliderType parentColliderType = ParentTilemap == null ? eColliderType.None : ParentTilemap.ColliderType;
				m_needsRebuildColliders = parentColliderType == eColliderType._3D && (m_meshCollider == null || m_meshCollider.sharedMesh == null);
                UpdateMesh();
                UpdateColliders();
            }
        }
...
//		private static Color[] gizmosColor = new Color[]{Color.red,Color.blue,Color.yellow,Color.green};
//		private void OnDrawGizmos()
//		{
//			if(OutlinePoints != null && OutlinePoints.Length >= 1)
//			{
//				int colorIndex = 0;
//				for(int i = 0;i < OutlinePoints.Length;i++)
//				{
//					Gizmos.color = gizmosColor[colorIndex];
//					Vector2[] points = OutlinePoints[i];
...
//						
//						Vector2 worldPosition = transform.TransformPoint(points[j]);
//						Vector2 worldPositionNex = transform.TransformPoint(nextPoint);
//						Gizmos.DrawLine(worldPosition,worldPositionNex);
//					}
//
//					colorIndex++;
//					if(colorIndex >= gizmosColor.Length)
//						colorIndex = 0;
//				}
//			}
//		}
