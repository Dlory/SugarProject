    ...
    partial class TilemapChunk
    {
        ...
        private bool FillColliderMeshData()
        {
            ...
            if (ParentTilemap.ColliderType == eColliderType._2D)
            {
				...
				if(ParentTilemap.autoCombineCollider)
				{
					Vector2[][] verts = new Vector2[openEdges.Count][];
					for(int i = 0;i < openEdges.Count;i++)
					{
						verts[i] = openEdges[i].ToArray();
					}

					verts = Polygon2DCombiner.CombinePolygons(verts);

					for (int i = 0; i < verts.Length;i++)
					{
						Vector2[] edgeSegments = verts[i];
						...
						if (ParentTilemap.Collider2DType == e2DColliderType.EdgeCollider2D)
						{
							((EdgeCollider2D)collider2D).points = edgeSegments;
						}
						else
						{
							((PolygonCollider2D)collider2D).points = Polygon2DCombiner.FlattenPoints(edgeSegments,ParentTilemap.flatternAngle,ParentTilemap.flatternDistance);
						}
					}
				}
				else
				{
					...
					for (int i = 0; i < openEdges.Count; ++i)
					{
						...
						{
							((PolygonCollider2D)collider2D).points = Polygon2DCombiner.FlattenPoints(edgeSegments.ToArray(),ParentTilemap.flatternAngle,ParentTilemap.flatternDistance);
						}
					}
				}
