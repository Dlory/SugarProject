            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                if (s_editMode == eEditMode.Renderer)
                {
                    ...
                    */
					...
					Color newColor = EditorGUILayout.ColorField("Color", m_tilemap.tintColor);
					if(newColor != m_tilemap.tintColor)
						m_tilemap.tintColor = newColor;
					m_tilemap.SetTintColor(newColor);
                ...
                else if (s_editMode == eEditMode.Collider)
                {
                    EditorGUI.BeginChangeCheck();
                    {
						...
						if(m_tilemap.ColliderType == eColliderType._2D && m_tilemap.Collider2DType == e2DColliderType.PolygonCollider2D)
						{
							m_tilemap.autoCombineCollider = EditorGUILayout.Toggle("Combine Collider",m_tilemap.autoCombineCollider);
							m_tilemap.flatternAngle = EditorGUILayout.FloatField("Flattern Angle",m_tilemap.flatternAngle);
							m_tilemap.flatternDistance = EditorGUILayout.FloatField("Flattern Distance",m_tilemap.flatternDistance);
						}
